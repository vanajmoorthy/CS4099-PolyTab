...
from model.Evaluation import pitch_precision, pitch_recall, pitch_f_measure, tab_precision, tab_recall, tab_f_measure, tab_disamb
...
if gpus:
    try:
        ...
        print(f"{len(gpus)} Physical GPUs, {
              len(tf.config.experimental.list_logical_devices('GPU'))} Logical GPUs")
    except RuntimeError as e:
        print(e)


class PolyTab:
    def __init__(self, batch_size=128, epochs=8, con_win_size=9, spec_repr="c", data_path="./cqt/", id_file="id.csv", save_path="./model/saved/"):
        ...
        self.num_strings = 6
        self.load_IDs()
        self.save_folder = os.path.join(self.save_path, f"{self.spec_repr} {
                                        datetime.datetime.now().strftime('%Y-%m-%d %H%M%S')}/")
        os.makedirs(self.save_folder, exist_ok=True)
        self.log_file = os.path.join(self.save_folder, "log.txt")
        self.metrics = {"pp": [], "pr": [], "pf": [], "tp": [], "tr": [], "tf": [
        ], "tdr": [], "data": ["g0", "g1", "g2", "g3", "g4", "g5", "mean", "std dev"]}

    def load_IDs(self):
        self.list_IDs = list(pd.read_csv(os.path.join(
            self.data_path, self.id_file), header=None)[0])

    def partition_data(self, data_split):
        self.partition = {"training": [], "validation": []}
        for ID in self.list_IDs:
            guitarist = int(ID.split("_")[0])
            self.partition["validation" if guitarist ==
                           data_split else "training"].append(ID)

        self.training_generator = DataGenerator(self.partition['training'], data_path=self.data_path,
                                                batch_size=self.batch_size, shuffle=True, spec_repr=self.spec_repr, con_win_size=self.con_win_size)
        self.validation_generator = DataGenerator(self.partition['validation'], data_path=self.data_path, batch_size=len(
            self.partition['validation']), shuffle=False, spec_repr=self.spec_repr, con_win_size=self.con_win_size)
    ...
    def build_model(self):
        ...
        model.compile(optimizer=AdamW(learning_rate=0.01, weight_decay=1e-4),
                      metrics=['accuracy'], loss=loss_layer.call)
        self.model = model
    ...
    def train(self):
        ...
        self.model.fit(self.training_generator, epochs=self.epochs, verbose=1, callbacks=[
                       model_checkpoint_callback, tensorboard_callback], use_multiprocessing=True, workers=9)
    ...
    def save_predictions(self):
        np.savez(os.path.join(self.split_folder, "predictions.npz"),
