...
class TabDataReprGen:
    ...
    def __init__(self, mode="c"):
        ...
        # representation and its labels storage
        self.output = {}
    ...
    def load_rep_and_labels_from_raw_file(self, filename):
        file_audio = self.path_audio + filename + "_hex_cln.wav"
    ...
    def preprocess_audio(self, data):
        ...
        if self.preproc_mode == "c":
            data = np.abs(librosa.cqt(data, hop_length=self.hop_length, sr=self.sr_curr, n_bins=self.cqt_n_bins, bins_per_octave=self.cqt_bins_per_octave))
            ...
            cqt = np.abs(librosa.cqt(data, hop_length=self.hop_length, sr=self.sr_curr, n_bins=self.cqt_n_bins, bins_per_octave=self.cqt_bins_per_octave))
    ...
    def get_nth_filename(self, n):
        ...
        filenames = list(filter(lambda x: x.endswith(".jams"), filenames))
...
def main(index, mode):
    # args is expected to be a list with two elements: [index, mode]
    gen = TabDataReprGen(mode=mode)
    gen.load_and_save_repr_nth_file(index)
